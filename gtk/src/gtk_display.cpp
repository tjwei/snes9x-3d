    ...
    if (endianess == ENDIAN_MSB)
    {
        ...
        else if (bpp == 24)
        {
            ...
            for (register int y = 0; y < height; y++)
            {
                ...
                for (register int x = 0; x < width; x++)
                {
                    //printf("%d %d src_pitch=%d dst_pitch=%d %p %p\n",x,y, src_pitch,dst_pitch, dst_buffer, src_buffer);
                    uint32 pixel = *snes++;
...
S9xDeinitUpdate (int width, int height)
{
...

    {
        ...
        {
            //SSSS
            S9xMergeHires (GFX.Screen1,
                           S9xDisplayDriver::image_width *
                               S9xDisplayDriver::image_bpp,
                           width,
                           height);
            S9xMergeHires (GFX.Screen2,
                           S9xDisplayDriver::image_width *
                               S9xDisplayDriver::image_bpp,
                           width,
                           height);
        ...
        //SSSS
        GFX.Screen1 = driver->get_next_buffer1 ();
        GFX.Screen2 = driver->get_next_buffer2 ();
    }
...
{
    uint16 *buffer1 = NULL, *buffer2=NULL;
    int    width, height;
    //SSSSSS
    buffer1 = (uint16 *) malloc (S9xDisplayDriver::image_size);
    buffer2 = (uint16 *) malloc (S9xDisplayDriver::image_size);
    memmove (buffer1,
             driver->get_current_buffer1 (),
             S9xDisplayDriver::image_size);
    memmove (buffer2,
             driver->get_current_buffer2 (),
             S9xDisplayDriver::image_size);
    ...
    driver->push_buffer (buffer1, buffer2);
