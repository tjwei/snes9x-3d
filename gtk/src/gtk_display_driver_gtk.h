...
class S9xGTKDisplayDriver : public S9xDisplayDriver
{
    public:
        ...
        uint16 *get_next_buffer1 (void);
        uint16 *get_current_buffer1 (void);
        uint16 *get_next_buffer2 (void);
        uint16 *get_current_buffer2 (void);
        void push_buffer (uint16 *src1, uint16 *src2);
        void reconfigure (int width, int height);

    private:
        void clear (void);
        void output (void *src1,
                      void *src2,
                     int src_pitch,
                     int x,
                     int y,
