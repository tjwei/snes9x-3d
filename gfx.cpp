...
bool8 S9xGraphicsInit (void)
{
	...
	GFX.SubScreen1  = ((uint16 *) malloc((GFX.ScreenSize+1024) * sizeof(uint16)))+512;
    GFX.SubScreen2  = ((uint16 *) malloc((GFX.ScreenSize+1024) * sizeof(uint16)))+512;
	GFX.ZBuffer1    = ((uint8 *)  malloc(GFX.ScreenSize+1024))+512;
    GFX.ZBuffer2    = ((uint8 *)  malloc(GFX.ScreenSize+1024))+512;
	GFX.SubZBuffer1 = ((uint8 *)  malloc(GFX.ScreenSize+1024))+512;
    GFX.SubZBuffer2 = ((uint8 *)  malloc(GFX.ScreenSize+1024))+512;

	if (!GFX.X2 || !GFX.ZERO || !GFX.SubScreen1 || !GFX.ZBuffer1 || !GFX.SubZBuffer1)
	{
		S9xGraphicsDeinit();
		return (FALSE);
	}
    if (!GFX.X2 || !GFX.ZERO || !GFX.SubScreen2 || !GFX.ZBuffer2 || !GFX.SubZBuffer2)
	{
		S9xGraphicsDeinit();
		return (FALSE);
	}
...
void S9xGraphicsDeinit (void)
{
	...
	if (GFX.SubScreen1)  { free(GFX.SubScreen1);  GFX.SubScreen1  = NULL; }
    if (GFX.SubScreen2)  { free(GFX.SubScreen2);  GFX.SubScreen2  = NULL; }
	if (GFX.ZBuffer1)    { free(GFX.ZBuffer1);    GFX.ZBuffer1    = NULL; }
	if (GFX.SubZBuffer1) { free(GFX.SubZBuffer1); GFX.SubZBuffer1 = NULL; }
    if (GFX.ZBuffer2)    { free(GFX.ZBuffer2);    GFX.ZBuffer2    = NULL; }
	if (GFX.SubZBuffer2) { free(GFX.SubZBuffer2); GFX.SubZBuffer2 = NULL; }
}
...
void S9xStartScreenRefresh (void)
{
	if (IPPU.RenderThisFrame)
	{
		...
		ZeroMemory(GFX.ZBuffer1-512, GFX.ScreenSize+1024);
		ZeroMemory(GFX.SubZBuffer1-512, GFX.ScreenSize+1024);
        ZeroMemory(GFX.ZBuffer2-512, GFX.ScreenSize+1024);
		ZeroMemory(GFX.SubZBuffer2-512, GFX.ScreenSize+1024);
	}
	...
	if (IPPU.RenderThisFrame)
	{
		...
		else
		{
			...
			if (Settings.AutoDisplayMessages){
				S9xDisplayMessages(GFX.Screen1, GFX.RealPPL, IPPU.RenderedScreenWidth, IPPU.RenderedScreenHeight, 1);
                S9xDisplayMessages(GFX.Screen2, GFX.RealPPL, IPPU.RenderedScreenWidth, IPPU.RenderedScreenHeight, 1);
            }
...
static inline void RenderScreen (bool8 sub)
{
	...
	if (!sub)
	{
		...
		GFX.DB1 = GFX.ZBuffer1;
        GFX.DB2 = GFX.ZBuffer2;
	...
	{
        //SSSSSS
		GFX.S1 = GFX.SubScreen1;
        GFX.S2 = GFX.SubScreen2;;
		GFX.DB1 = GFX.SubZBuffer1;
        GFX.DB2 = GFX.SubZBuffer2;
...
void S9xUpdateScreen (void)
{
		...
		if (Settings.SupportHiRes)
		{
			if (!IPPU.DoubleWidthPixels && (PPU.BGMode == 5 || PPU.BGMode == 6 || IPPU.PseudoHires))
			{
			...
			#endif
				{
					...
					{
						register uint16	*p = GFX.Screen1 + y * GFX.PPL + 255;
						register uint16	*q = GFX.Screen1 + y * GFX.PPL + 510;
                    ...
                    for (register uint32 y = 0; y < GFX.StartY; y++)
					{
						register uint16	*p = GFX.Screen2 + y * GFX.PPL + 255;
						register uint16	*q = GFX.Screen2 + y * GFX.PPL + 510;
			...
			if (!IPPU.DoubleHeightPixels && IPPU.Interlace)
			{
				...
				for (register int32 y = (int32) GFX.StartY - 1; y >= 0; y--){
					memmove(GFX.Screen1 + y * GFX.PPL, GFX.Screen1 + y * GFX.RealPPL, IPPU.RenderedScreenWidth * sizeof(uint16));
                    memmove(GFX.Screen2 + y * GFX.PPL, GFX.Screen2 + y * GFX.RealPPL, IPPU.RenderedScreenWidth * sizeof(uint16));
                }
			}
			else if (IPPU.DoubleHeightPixels && !IPPU.Interlace)
			{
				for (register int32 y = 0; y < (int32) GFX.StartY; y++){
					memmove(GFX.Screen1 + y * GFX.RealPPL, GFX.Screen1 + y * GFX.PPL, IPPU.RenderedScreenWidth * sizeof(uint16));
                    memmove(GFX.Screen2 + y * GFX.RealPPL, GFX.Screen2 + y * GFX.PPL, IPPU.RenderedScreenWidth * sizeof(uint16));
                }
	...
	else
	{
		...
		GFX.S1 = GFX.Screen1 + GFX.StartY * GFX.PPL;
        GFX.S2 = GFX.Screen2 + GFX.StartY * GFX.PPL;
		if (GFX.DoInterlace && GFX.InterlaceFrame){
			GFX.S1 += GFX.RealPPL;
            GFX.S2 += GFX.RealPPL;
        }

		for (uint32 l = GFX.StartY; l <= GFX.EndY; l++, GFX.S1 += GFX.PPL, GFX.S2 += GFX.PPL)
			for (int x = 0; x < IPPU.RenderedScreenWidth; x++)
				GFX.S1[x]=GFX.S2[x] = black;
	}
...
static void DisplayStringFromBottom (const char *string, int linesFromBottom, int pixelsFromLeft, bool allowWrap)
{
	...
	uint16	*dst1 = GFX.Screen1 + (IPPU.RenderedScreenHeight - font_height * linesFromBottom) * GFX.RealPPL + pixelsFromLeft;
    uint16	*dst2 = GFX.Screen2 + (IPPU.RenderedScreenHeight - font_height * linesFromBottom) * GFX.RealPPL + pixelsFromLeft;
	...
	for (int i = 0 ; i < len ; i++, char_count++)
	{
		if (char_count >= max_chars || (uint8) string[i] < 32)
		{
			...
			dst1 += font_height * GFX.RealPPL - (font_width - 1) * max_chars;
            dst2 += font_height * GFX.RealPPL - (font_width - 1) * max_chars;
			if (dst1 >= GFX.Screen1 + IPPU.RenderedScreenHeight * GFX.RealPPL)
				break;
		...
		S9xDisplayChar(dst1, string[i]);
        S9xDisplayChar(dst2, string[i]);
		dst1 += font_width - 1;
        dst2 += font_width - 1;
	}
}
...
#endif
	{
		uint16	*s1 = GFX.Screen1 + y * GFX.RealPPL + x;
        uint16	*s2 = GFX.Screen2 + y * GFX.RealPPL + x;

		for (r = 0; r < 15 * rx; r++, s1 += GFX.RealPPL - 15 * cx, s2 += GFX.RealPPL - 15 * cx)
		{
			if (y + r < 0)
			{
				s1 += 15 * cx;
			...
			for (c = 0; c < 15 * cx; c++, s1++, s2++)
			{
				if (x + c < 0 || s1 < GFX.Screen1)
					continue;
				...
				if (p == '#' && fgcolor){
					*s1 = (fgcolor & 0x10) ? COLOR_ADD1_2(fg, *s1) : fg;
                    *s2 = (fgcolor & 0x10) ? COLOR_ADD1_2(fg, *s2) : fg;
                }
				else
				if (p == '.' && bgcolor){
					*s1 = (bgcolor & 0x10) ? COLOR_ADD1_2(*s1, bg) : bg;
                    *s2 = (bgcolor & 0x10) ? COLOR_ADD1_2(*s2, bg) : bg;
                }
			}
		}
	}
